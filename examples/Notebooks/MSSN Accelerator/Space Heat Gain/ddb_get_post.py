...
def add_new_source(
    ...
    a = get_sources(reference_id=project_id)  # get existing sources
    ...
    for i, s in enumerate(
        ...
        b = [
            x["id"]
            for x in a
            if x["title"] == source_title[i] and x["reference"] == source_reference[i]
        ]

        if b != []:
            source_ids.append(b[0])  # if it exists, find its id
            ...
            new_source = post_body(endpoint="sources/", body=source)

            if "details" in new_source:
                print(f"Error: {new_source['details']}")
            new_source_id = new_source["source"]["id"]
...
def add_new_parameters(
    ...
    chunks = [l[i : i + 12] for i in range(0, len(l), 12)]
